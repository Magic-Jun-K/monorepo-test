...
// 创建useMessage hook
export const useMessage = (): [MessageInstance, React.ReactElement] => {
  ...
  const messageApi = useMemo((): MessageInstance => {
    ...
    return {
      ...
      info: config => addMessage(typeof config === 'string' ? { content: config } : config, 'info'),
      loading: config => {
        const id = Math.random().toString(36).substring(2, 11);
  ...
  const MessageComponent = React.createElement(
    ...
    { value: messageApi },
    React.createElement(MessageInner, { messages: messages })
  );
...
// 内部组件，用于渲染消息
const MessageInner: React.FC<{ messages: MessageItem[] }> = ({ messages }) => {
  const containerRef = useRef<HTMLDivElement>(null);

  return React.createElement(
    'div',
    { ref: containerRef, className: 'fixed top-4 left-1/2 transform -translate-x-1/2 z-50' },
    messages.map(msg =>
      React.createElement(
        'div',
        {
