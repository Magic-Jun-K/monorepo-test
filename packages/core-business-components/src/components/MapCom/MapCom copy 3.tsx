...
// import { getWasmUrl } from '@/utils/loadWasm';
...
//       // 智能获取 wasm 路径并传递给 worker
//       getWasmUrl('release.wasm')
//         .then(wasmUrl => {
//           console.log('获取到WASM URL:', wasmUrl);
//           workerRef.current?.postMessage({ type: 'initWasm', wasmUrl });
//         })
//         .catch(error => {
//           console.warn('获取WASM URL失败，使用默认路径:', error);
//           // 使用默认路径作为备选方案
//           const defaultWasmUrl = new URL('/wasm/release.wasm', window.location.origin).toString();
//           workerRef.current?.postMessage({ type: 'initWasm', wasmUrl: defaultWasmUrl });
//         });
