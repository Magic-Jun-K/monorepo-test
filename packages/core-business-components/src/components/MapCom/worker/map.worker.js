...
const generateRandomCoordinatesJS = (minLng, maxLng, minLat, maxLat, count) => {
    const points = [];
...
// 使用WASM生成坐标点
const generateRandomCoordinatesWasm = (minLng, maxLng, minLat, maxLat, count) => {
    ...
    try {
        ...
        // });
        // 调用WASM函数，返回内存指针
        const ptr = wasmModule.generateRandomPoints(safeCount, safeMinLng, safeMaxLng, safeMinLat, safeMaxLat);
        ...
        if (remaining > 0 && remaining < count && actualCount > 0) {
            // 只有在递归参数完全正常时才递归
            const remainingPoints = generateRandomCoordinatesWasm(minLng, maxLng, minLat, maxLat, remaining);
            points.push(...remainingPoints);
        }
        else if (remaining > 0) {
            // 只要递归参数异常，直接用JS补齐
            console.error(`WASM递归出口防御触发：count=${count}, actualCount=${actualCount}, remaining=${remaining}，回退到JS实现`);
...
self.onmessage = (e) => {
    ...
    if (e.data.type === 'generatePoints') {
        // 现有的处理逻辑...
        const { count, bounds: [minLng, maxLng, minLat, maxLat], batchSize = 50000 // 默认批次大小
         } = e.data;
        // 计算需要的批次数
        ...
        (async () => {
            ...
            for (let batch = 0; batch < batchCount; batch++) {
                ...
                if (currentBatchSize <= 0)
                    continue;
                // 生成当前批次的点，考虑视口优先
                const batchPoints = generatePointsWithViewportPriority(minLng, maxLng, minLat, maxLat, currentBatchSize, currentViewport);
                // 创建TypedArray用于传输
                const pointsBuffer = new Float64Array(currentBatchSize * 2);
                for (let i = 0; i < currentBatchSize; i++) {
                    pointsBuffer[i * 2] = (_b = (_a = batchPoints[i]) === null || _a === void 0 ? void 0 : _a.lng) !== null && _b !== void 0 ? _b : 0;
                    pointsBuffer[i * 2 + 1] = (_d = (_c = batchPoints[i]) === null || _c === void 0 ? void 0 : _c.lat) !== null && _d !== void 0 ? _d : 0;
                }
                // 发送当前批次
                self.postMessage({
                    type: 'pointsBatch',
...
const generatePointsWithViewportPriority = (minLng, maxLng, minLat, maxLat, count, viewport) => {
    if (count <= 0)
        return [];
    // 如果没有视口信息，使用普通生成
    if (!viewport) {
