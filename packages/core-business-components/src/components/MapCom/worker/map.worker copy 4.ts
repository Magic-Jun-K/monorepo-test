...
self.onmessage = (e: MessageEvent<WorkerMessage>) => {
  if (e.data.type === 'generatePoints') {
    ...
    // 使用WASM或备用JS生成点
    const points: { lng: number; lat: number }[] = wasmModule
    ...
    // 创建TypedArray用于传输
    const pointsBuffer = new Float64Array(count * 2);
    for (let i = 0; i < count; i++) {
      pointsBuffer[i * 2] = points[i]?.lng ?? 0; // 使用空值合并运算符，当值为undefined时使用默认值0
      pointsBuffer[i * 2 + 1] = points[i]?.lat ?? 0;
    }

    // 使用Transferable Objects传输数据
    self.postMessage(
      {
        type: 'pointsBuffer',
