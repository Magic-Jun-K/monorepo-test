...
const LoginContent = () => {
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      // 类型守卫
      if (isAccountOrRegister(formData)) {
        response = await handleAccountOrRegister(formData); // 此时 formData 类型为 LoginFormData | RegisterFormData
      } else {
        response = await handleEmailLogin(formData); // 此时 formData 类型为 EmailLoginFormData
      }
