  ...
  imports: [
    ...
    NestRedisModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => {
        ...
        // Fallback logic: prefer config, then env（备用逻辑：优先使用配置文件，然后使用环境变量。）
        const password = redisConf?.connection?.password || envPassword;
        const host = redisConf?.connection?.url || 'localhost';
        const port = redisConf?.connection?.port || 6379;
        const db = redisConf?.connection?.db || 0;
