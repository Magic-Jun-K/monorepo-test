...
// import { register, login, emailLogin, sendVerificationCode } from '@/services/auth';
// import {
//   AuthType,
//   LoginType,
//   FormData,
...
//         if (response.data) {
//           // 如果data是对象且包含accessToken属性
//           if (typeof response.data === 'object' && response.data !== null && 'accessToken' in response.data) {
//             authStore.setToken((response.data as { accessToken: string }).accessToken);
...
//         // 特别处理账户锁定情况
//         if (status === 429 || data?.code === 'ACCOUNT_LOCKED') {
//           addToast({
//             message: data?.message || '账户已被锁定，请15分钟后重试',
//             type: 'error'
//           });
//         } else if (status === 401) {
//           addToast({ message: '用户名或密码错误', type: 'error' });
//         } else if (status === 403) {
...
//     });
//     // if (authType === 'login') {
//     //   return await login(data.username, encryptedPassword);
//     // } else {
...
//           addToast({ message: '验证码已发送，请查收邮箱', type: 'success' });
//         } else {
//           addToast({ message: response.message || '验证码发送失败，请稍后重试', type: 'error' });
//           setCountdown(0);
//         }
//       })
//       .catch(() => {
